...
stdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [
    ...
    cmake
    pkg-config
  ] ++ lib.lists.optionals withInternalQEMU [ ninja ];
  buildInputs =
    [
      # Xen
      acpica-tools
  ...
  env.NIX_CFLAGS_COMPILE =
    ...
    + lib.strings.optionalString withInternalOVMF " -Wno-error=format-security -Wno-error=use-after-free -Wno-error=vla-parameter -Wno-error=dangling-pointer -Wno-error=stringop-overflow";
  ...
  buildFlags = [
    "xen" # Build the hypervisor.
    "tools" # Build the userspace tools.
  ];

  postBuild = ''
    make --directory docs/ man-pages
  ...
  installPhase =
    let
      cpFlags = builtins.concatStringsSep " " [
    ...
    # We also need to wrap pygrub, which lies in $out/lib.
    + ''
