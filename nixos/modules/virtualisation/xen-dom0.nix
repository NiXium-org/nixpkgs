...

{
  ...
  options = {
    ...
    virtualisation.xen = {
      ...
      efi = {
        ...
        # TODO: Deprecate this when the EFI configuration is generalised into Bootspec.
        bootEntryStyle = lib.options.mkOption {
          type = lib.types.enum [
            "simple"
            "traditional"
            "detailed"
          ...
          default = "simple";
          description = ''
            Due to upstream `systemd-boot` and Xen limitations, only one boot entry
            may be configured. This option defines the title that boot entry will
            use, but does not alter any boot logic.

            - `simple` names the entry "Xen Hypervisor".

            - `traditional` names the entry "NixOS ${lib.trivial.release} (with Xen Hypervisor)",
              following the traditional naming style for Xen distributions.

            - `detailed` names the entry "NixOS ${lib.trivial.codeName} ${lib.trivial.release}.[Nixpkgs Revision] (Linux [Kernel Variant]-[Kernel Version]) (Xen [Xen Version])",
              resulting in a long but verbose entry.
          '';
        };

        path = lib.options.mkOption {
          type = lib.types.path;
          default = "${cfg.package.boot}/boot/xen-${cfg.package.version}.efi";
          defaultText = lib.options.literalExpression "\${config.virtualisation.xen.package.boot}/boot/xen-\${config.virtualisation.xen.package.version}.efi";
  ...
  config = lib.modules.mkIf cfg.enable {
    ...
    boot = {
      ...
      # TODO: This is a temporary solution until we can switch to bootspec/lzbt
      loader.systemd-boot = lib.modules.mkIf cfg.efi.enable {
        extraEntries = {
          "xen.conf" =
            "title "
            + lib.strings.optionalString (cfg.efi.bootEntryStyle == "simple") "Xen Hypervisor"
            + lib.strings.optionalString (
              cfg.efi.bootEntryStyle == "traditional"
            ) "${config.system.nixos.distroName} ${config.system.nixos.release} (with Xen Hypervisor)"
            +
              lib.strings.optionalString (cfg.efi.bootEntryStyle == "detailed")
                "${config.system.nixos.distroName} ${config.system.nixos.codeName} ${config.system.nixos.version} (Linux ${config.boot.kernelPackages.kernel.modDirVersion}) (Xen ${cfg.package.version})"
            + ''

              efi /efi/nixos/xen.efi
              sort-key a_xen
            '';
        };
        extraFiles = {
          "efi/nixos/xen.efi" = "${cfg.efi.path}";
          "efi/nixos/xen.cfg" = "${pkgs.writeText "xen.cfg" ''
            [global]
            default=xen

            [xen]
            options=${toString cfg.bootParams}
            kernel=${lib.removePrefix "/nix/store/" config.system.build.kernel.outPath}-${config.system.boot.loader.kernelFile}.efi root=fstab init=INIT_PATH ${toString config.boot.kernelParams}
            ramdisk=${lib.removePrefix "/nix/store/" config.system.build.initialRamdisk.outPath}-${config.system.boot.loader.initrdFile}.efi
          ''}";
        };
      };
    };
